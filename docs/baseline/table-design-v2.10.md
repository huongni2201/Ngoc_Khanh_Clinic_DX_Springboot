...
Version: 2.10 — baseline 22/09/2026

Source of truth: requirement-v2.4 + use-case-v2.5 + uploaded/current clinic print templates

Status: synchronized baseline for pre-check-in preparation, off-site corporate examinations, manager-only employee import and uniform batch repricing
...
- SQL Server-compatible types are used in examples: uniqueidentifier, uniqueidentifier, datetime2(3), decimal(18,2), nvarchar, varchar and rowversion.
...
Purpose: Canonical patient record. identificationNumber is the only supported legal/business identifier in the baseline.
...
- identificationNumber is mandatory and is the only identity document supported by the baseline.

- identificationNumber is stored directly on patients as the sole supported patient identifier.
...
| performing_department_id | bigint | NULL REFERENCES departments(id) |
| default_room_id | bigint | NULL REFERENCES rooms(id) |
...
| lab_panel_id | bigint | NULL REFERENCES lab_panels(id) |
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| document_template_id | bigint | NOT NULL REFERENCES document_templates(id) |
| version_number | int | NOT NULL |
| source_file_attachment_id | bigint | NOT NULL REFERENCES file_attachments(id) |
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| document_template_version_id | bigint | NOT NULL REFERENCES document_template_versions(id) |
...
| file_attachment_id | bigint | NULL REFERENCES file_attachments(id) |
| generated_by_user_id | bigint | NULL REFERENCES users(id) |
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| generated_document_id | bigint | NOT NULL REFERENCES generated_documents(id) |
...
| patient_id | bigint | NOT NULL REFERENCES patients(id) |
| source_encounter_id | bigint | NULL REFERENCES encounters(id) |
| department_id | bigint | NULL REFERENCES departments(id) |
| doctor_staff_id | bigint | NULL REFERENCES staff(id) |
...
| checked_in_by_user_id | bigint | NULL REFERENCES users(id); actor of actual check-in |
...
Purpose: Current patient journey state and destination location used to drive destination-room/doctor worklists.
...
| current_department_id | bigint | NULL REFERENCES departments(id) |
| current_room_id | bigint | NULL REFERENCES rooms(id) |
...
- current_department_id/current_room_id represent the Encounter's current routing destination.

- Worklists for destination rooms/doctors are derived from current_stage + current location/assignment; no returned-to-doctor confirmation flag or return queue is stored.

- A PREPARED Encounter remains at REGISTERED and is excluded from actionable worklists until authorized check-in. Off-site desk/zone is resolved from the active encounter assignment and batch examination site.

Indexes:

- IX_journeys_stage(current_stage, current_room_id, stage_entered_at)

### 4.22 journey_events

Purpose: Immutable journey-stage history.
...
- IX_journey_events_journey(journey_id, occurred_at)
...
| recorded_by_staff_id | bigint | NULL REFERENCES staff(id) |
...
| author_staff_id | bigint | NOT NULL REFERENCES staff(id) |
| status | varchar(16) | NOT NULL |
| created_at datetime2(3) NOT NULL |  |  |
...
| id | bigint | IDENTITY PRIMARY KEY |
| encounter_id | bigint | NOT NULL REFERENCES encounters(id) |
| diagnosis_catalog_id | bigint | NULL REFERENCES diagnosis_catalog(id) |
| diagnosis_text | nvarchar(500) | NOT NULL |
| diagnosis_type | varchar(24) | NOT NULL |
| is_primary | bit | NOT NULL DEFAULT 0 |
| recorded_by_staff_id | bigint | NOT NULL REFERENCES staff(id) |
| recorded_at datetime2(3) NOT NULL |  |  |
...
| id | bigint | IDENTITY PRIMARY KEY |
| encounter_id | bigint | NOT NULL REFERENCES encounters(id) |
| health_check_record_id | bigint | NULL REFERENCES health_check_records(id) |
| round_number | int | NOT NULL |
| source_type | varchar(24) | NOT NULL |
| status | varchar(16) | NOT NULL |
| ordered_by_staff_id | bigint | NOT NULL REFERENCES staff(id) |
| created_by_user_id | bigint | NOT NULL REFERENCES users(id) |
...
| id | bigint | IDENTITY PRIMARY KEY |
| public_id | uniqueidentifier | NOT NULL UNIQUE |
| order_round_id | bigint | NOT NULL REFERENCES order_rounds(id) |
| service_id | bigint | NOT NULL REFERENCES services(id) |
| status | varchar(16) | NOT NULL |
| priority | varchar(16) | NOT NULL DEFAULT 'ROUTINE' |
| performing_department_id | bigint | NULL REFERENCES departments(id) |
| performing_room_id | bigint | NULL REFERENCES rooms(id) |
...
| service_request_id | bigint | NULL REFERENCES service_requests(id) |
...
| confirmed_by_user_id | bigint | NULL REFERENCES users(id) |
...
| patient_id | bigint | NULL REFERENCES patients(id); linked/resolved at visit preparation, not required at roster import |
...
### - Employee import does not create Patient or Encounter. At preparation, resolve/create Patient by exact identificationNumber and link patient_id. Re-import must not silently relink an employee to a different Patient or overwrite Patient master. Identity conflicts require review before issuing forms.
...
| master_template_version_id | bigint | NOT NULL REFERENCES document_template_versions(id) |
...
### 4.36 health_check_batch_services
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| health_check_batch_id | bigint | NOT NULL REFERENCES health_check_batches(id) |
| service_id | bigint | NOT NULL REFERENCES services(id) |
| document_template_version_id | bigint | NULL REFERENCES document_template_versions(id) |
| service_code_snapshot | varchar(40) | NOT NULL |
| service_name_snapshot | nvarchar(300) | NOT NULL |
...
### 4.37 health_check_batch_employees
...
| public_id | uniqueidentifier | NOT NULL UNIQUE |
| health_check_batch_id | bigint | NOT NULL REFERENCES health_check_batches(id) |
| company_employee_id | bigint | NOT NULL REFERENCES company_employees(id) |
...
| administrative_snapshot_json | nvarchar(max) | NOT NULL; validated versioned-schema object containing all confirmed import administrative fields for this employee/batch, including identificationNumber, name, DOB, sex and optional Form 03 fields |
...
| patient_id | bigint | NOT NULL REFERENCES patients(id) |
| health_check_batch_employee_id | bigint | NULL REFERENCES health_check_batch_employees(id) |
| encounter_id | bigint | NOT NULL REFERENCES encounters(id) |
| master_template_version_id | bigint | NOT NULL REFERENCES document_template_versions(id) |
...
| replaces_health_check_record_id | bigint | NULL REFERENCES health_check_records(id) |
| created_at | datetime2(3) | NOT NULL |
| completed_at | datetime2(3) | NULL |
| canceled_at | datetime2(3) | NULL |
| row_version | rowversion |  |
...
### - No identity_type/passport model is introduced; baseline uses identificationNumber only.
...
### 4.39 health_check_batch_employee_services
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| health_check_batch_employee_id | bigint | NOT NULL REFERENCES health_check_batch_employees(id) |
| health_check_batch_service_id | bigint | NOT NULL REFERENCES health_check_batch_services(id) |
| service_request_id | bigint | NOT NULL REFERENCES service_requests(id) |
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| health_check_import_job_id | bigint | NOT NULL REFERENCES health_check_import_jobs(id) |
...
| resolved_patient_id | bigint | NULL REFERENCES patients(id) |
| resolved_company_employee_id | bigint | NULL REFERENCES company_employees(id) |
| resolved_batch_employee_id | bigint | NULL REFERENCES health_check_batch_employees(id) |
| resolved_batch_service_id | bigint | NULL REFERENCES health_check_batch_services(id) |
| resolved_service_request_id | bigint | NULL REFERENCES service_requests(id) |

### Rules / Constraints:

### - UNIQUE(health_check_import_job_id, row_number).

### - EMPLOYEE_LIST requires employeeCode, valid identificationNumber, full name, DOB and sex. Resolve CompanyEmployee/BatchEmployee; resolved_patient_id may remain NULL until a separate preparation command.

### - RESULTS requires serviceCode and at least one of employeeCode or identificationNumber.
...
| collected_by_staff_id | bigint | NULL REFERENCES staff(id) |
| received_at datetime2(3) NULL |  |  |

### 4.43 specimen_service_requests
...
| analyte_id | bigint | NOT NULL REFERENCES analytes(id) |
| display_order | int | NOT NULL |
...
| public_id | uniqueidentifier | NOT NULL UNIQUE |
| service_request_id | bigint | NOT NULL REFERENCES service_requests(id) |
| specimen_id | bigint | NULL REFERENCES specimens(id) |
| version_number | int | NOT NULL |
| status | varchar(16) | NOT NULL |
| supersedes_lab_result_id | bigint | NULL REFERENCES lab_results(id) |
| result_source | varchar(24) | NOT NULL |
| verified_by_staff_id | bigint | NULL REFERENCES staff(id) |
...
| public_id | uniqueidentifier | NOT NULL UNIQUE |
| service_request_id | bigint | NOT NULL REFERENCES service_requests(id) |
| imaging_study_id | bigint | NULL REFERENCES imaging_studies(id) |
| document_template_version_id | bigint | NULL REFERENCES document_template_versions(id) |
| version_number | int | NOT NULL |
| status | varchar(16) | NOT NULL |
...
| supersedes_report_id | bigint | NULL REFERENCES diagnostic_reports(id) |
| author_staff_id | bigint | NOT NULL REFERENCES staff(id) |
| verified_by_staff_id | bigint | NULL REFERENCES staff(id) |
| finalized_at datetime2(3) NULL |  |  |
| created_at datetime2(3) NOT NULL |  |  |
...
| encounter_id | bigint | NOT NULL REFERENCES encounters(id) |
| patient_id | bigint | NOT NULL REFERENCES patients(id) |
| prescriber_staff_id | bigint | NOT NULL REFERENCES staff(id) |
| version_number | int | NOT NULL |
| status | varchar(16) | NOT NULL |
| supersedes_prescription_id | bigint | NULL REFERENCES prescriptions(id) |
...
| medication_id | bigint | NULL REFERENCES medications(id) |
...
| --- | --- | --- |
| id | bigint | IDENTITY PRIMARY KEY |
| integration_endpoint_id | bigint | NOT NULL REFERENCES integration_endpoints(id) |
...
ISSUED -> CORRECTED as a new version
...
2. Validate roster fields, exact identificationNumber/employee-code matching and duplicates. Do not create Patient or Encounter during import.
...
1. For the selected employee and planned examination date, authorize preparation and validate adult eligibility. Exact lookup Patient by roster identificationNumber, create when absent and link CompanyEmployee. Resolve an existing valid prepared visit or create HEALTH_CHECK Encounter PREPARED, Journey REGISTERED and HealthCheckRecord with mandatory Patient/Encounter in an idempotent command.
...
3. Atomically and idempotently change Encounter PREPARED to IN_PROGRESS, set checked_in_at/checked_in_by_user_id/started_at and HealthCheckRecord.actual_examination_date; update Journey/assignment for the authorized reception flow.
...
Front Desk resolves/creates Patient, creates Encounter/Journey, creates initial consultation invoice and collects payment/authorization before Doctor Worklist.
...
Payment confirmation authorizes eligible ServiceRequests and moves Journey to WAITING_FOR_DIAGNOSTIC.

8.4 Final Result

Final result is versioned, updates ServiceRequest completion/readiness and can move Journey to WAITING_FOR_CONCLUSION.

8.5 Complete Encounter

Doctor finalizes diagnosis/prescription/follow-up and completes Encounter/Journey.
...
- journeys(current_stage, current_room_id, stage_entered_at)
...
- Every new HEALTH_CHECK visit receives one administrative snapshot at preparation, before or at check-in. Name, DOB, sex and identificationNumber are mandatory; other fields follow business nullability. Reprint uses the stored snapshot. A new print/check-in command does not itself mean a new visit.
